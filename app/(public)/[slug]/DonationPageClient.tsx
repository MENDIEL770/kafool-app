...
// Floating WhatsApp button. Portaled to <body> so no ancestor transform can
// reparent its fixed positioning, pinned to the bottom-right with plain physical
// offsets and no transform/transition — so it can never drift or jump sides.
// Rendered only after mount (`show`) to keep SSR output clean.
function WhatsAppFab({ url, show }: { url: string | null; show: boolean }) {
  if (!show || !url || typeof document === 'undefined') return null
  return createPortal(
    <a
      href={url}
      target="_blank"
      ...
      aria-label="WhatsApp"
      dir="ltr"
      className="fixed z-[60] flex h-14 w-14 items-center justify-center rounded-full shadow-xl"
      style={{ backgroundColor: '#25D366', right: '1rem', bottom: '6rem' }}
    >
...
export default function DonationPageClient({ org, campaign, donations: initialDonations, groups, gallery, activeGroup, donationUrl = '', paymentUrls, paymentProvider, nedarim, initialLang }: Props) {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-clip" dir="rtl" style={pageBg ? { backgroundColor: pageBg } : undefined}>
      ...
      {/* 8. Floating bar */}
      <WhatsAppFab url={whatsappUrl} show={mounted} />
