        ...
        {lbIdx !== null && lbImages[lbIdx] && typeof document !== 'undefined' && createPortal(
          ...
          >
            ...
            <button onClick={closeLb} aria-label="סגור"
              className="absolute z-20 top-4 left-4 w-10 h-10 rounded-full bg-white/90 hover:bg-white text-gray-800 shadow-lg flex items-center justify-center transition-colors">
              <X className="w-5 h-5" />
            </button>
            ...
            {lbImages.length > 1 && (
              <div className="absolute z-20 bottom-5 inset-x-0 flex items-center justify-center gap-3">
                <button type="button" aria-label="הקודם" onClick={lbPrev}
                  className="w-11 h-11 rounded-full bg-white/90 hover:bg-white text-gray-800 shadow-lg flex items-center justify-center transition-colors">
...
function FloatingBar({ primaryColor, buttonRadius, onDonate, whatsappUrl }: { campaign: Campaign; primaryColor: string; buttonRadius: string; donateHref?: string; onDonate: () => void; whatsappUrl?: string | null }) {
      ...
      <div className="max-w-lg mx-auto flex gap-2 px-4 py-3">
        ...
        {whatsappUrl && (
          <a
            ...
            aria-label="WhatsApp"
            className={`shrink-0 flex items-center justify-center px-4 py-3 shadow-md hover:opacity-90 transition-opacity ${buttonRadius}`}
        ...
        return (
          <div key={key} className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-white/60 px-4 py-3 flex items-center gap-3 select-none"
            ...
            <div className="min-w-0 flex-1">
              <p className="text-base font-black leading-tight flex items-baseline gap-6" style={{ color: primaryColor }}>
                <span>{donationAmount(d)}</span>
                {d.payment_type === 'hok' && d.monthly_amount && d.installments ? (
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setOpen(false)}>
      <div className="relative max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <button onClick={() => setOpen(false)} aria-label="סגור"
          className="absolute z-10 top-2 left-2 w-10 h-10 rounded-full bg-black/55 hover:bg-black/75 text-white flex items-center justify-center shadow-lg transition-colors">
          <X className="w-5 h-5" />
        </button>
        {ad.link
...
export default function DonationPageClient({ org, campaign, donations: initialDonations, groups, gallery, activeGroup, donationUrl = '', paymentUrls, paymentProvider, nedarim, initialLang }: Props) {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-clip" dir="rtl" style={pageBg ? { backgroundColor: pageBg } : undefined}>
      ...
      {/* 8. Floating bar (Donate · Share · WhatsApp) */}
      <FloatingBar campaign={campaign} primaryColor={primaryColor} buttonRadius={buttonRadius} onDonate={() => openDonate()} whatsappUrl={whatsappUrl} />
      ...
      <DonationModal
        ...
        disabledMethods={(campaign.settings as { payment?: { disabled?: string[] } })?.payment?.disabled || []}
