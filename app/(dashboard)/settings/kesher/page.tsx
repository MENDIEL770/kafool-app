...
export default function KesherSettingsPage() {
    ...
    async function load() {
      ...
      if (conn) {
        setForm({
          ...
          kesher_password: decryptPassword(conn.kesher_password_encrypted || ''),
          project_number: conn.project_number || '',
          payment_page_id: String(conn.payment_page_id || ''),
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const supabase = createClient()
    const encrypted = form.kesher_password ? encryptPassword(form.kesher_password) : ''

    await supabase.from('kesher_connections').upsert({
      org_id: orgId,
      kesher_username: form.kesher_username,
      kesher_password_encrypted: encrypted,
      project_number: form.project_number,
      payment_page_id: Number(form.payment_page_id),
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6" dir="rtl">
      ...
      <form onSubmit={handleSave} className="space-y-5">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input
                value={form.kesher_username}
                onChange={e => set('kesher_username', e.target.value)}
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <input
                  ...
                  value={form.kesher_password}
                  onChange={e => set('kesher_password', e.target.value)}
                  placeholder="••••••••"
                  dir="ltr"
                  required
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <input
                type="number"
                value={form.payment_page_id}
                onChange={e => set('payment_page_id', e.target.value)}
                placeholder="678"
                dir="ltr"
                required
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500"
        ...
        {form.kesher_username && form.payment_page_id && form.is_active && (
